...
describe('projectionRegistry contract gate', () => {
  ...
  it('fails fast and retains no incompatible companion projection', async () => {
    ...
    expect(registry.entries['execution-mismatch']?.lastError).toContain('unsupported execution projection schema_version 5');
  });
  ...
  it('stops a live consumer when a delta contract version changes', async () => {
    ...
    expect(registry.entries['execution-live']?.lastError).toContain('unsupported execution projection delta schema_version 5');
  });
