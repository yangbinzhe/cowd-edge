import { t } from '../index';

const statusKeys: Record<string, string> = {
  active: 'status.active',
  attention: 'status.attention',
  auto: 'status.auto',
  accepted: 'status.accepted',
  accepted_pending_materialization: 'status.queued',
  allowed: 'status.allowed',
  approved: 'status.approved',
  admitted: 'status.admitted',
  admitting: 'status.admitting',
  attached: 'status.attached',
  blocked: 'status.blocked',
  blocked_policy: 'status.blockedPolicy',
  builtin: 'status.builtin',
  candidate: 'status.candidate',
  calling_model: 'status.callingModel',
  calling_tool: 'status.callingTool',
  claimed: 'status.claimed',
  cancel_requested: 'status.cancelRequested',
  cancelled: 'status.cancelled',
  closed: 'status.closed',
  connected: 'status.connected',
  connecting: 'status.connecting',
  circuit_open: 'status.circuitOpen',
  complete: 'status.complete',
  completed: 'status.complete',
  completed_with_warnings: 'status.completedWithWarnings',
  completion_requested: 'status.completionRequested',
  config: 'status.config',
  degraded: 'status.degraded',
  denied: 'status.denied',
  declared: 'status.declared',
  delivered: 'status.delivered',
  draining: 'status.draining',
  dead_letter: 'status.deadLetter',
  disabled: 'status.disabled',
  dry_run: 'status.dryRun',
  done: 'status.done',
  empty: 'status.empty',
  error: 'status.error',
  evaluating: 'status.evaluating',
  events_visible: 'status.eventsVisible',
  fail: 'status.failed',
  failed: 'status.failed',
  first_output: 'status.firstOutput',
  forbidden: 'status.forbidden',
  finalizing: 'status.finalizing',
  high: 'status.high',
  healthy: 'status.healthy',
  indexed: 'status.indexed',
  initial: 'status.initial',
  info: 'status.info',
  idle: 'status.idle',
  invalid: 'status.invalid',
  invalid_response: 'status.invalidResponse',
  missing: 'status.missing',
  missing_resources: 'status.missingResources',
  missing_routes: 'status.missingRoutes',
  no_events: 'status.noEvents',
  not_found: 'status.notFound',
  not_tested: 'status.notTested',
  observed: 'status.observed',
  offline: 'status.offline',
  timeout: 'status.timeout',
  ok: 'status.ok',
  open: 'status.open',
  optional: 'status.optional',
  pass: 'status.pass',
  pending: 'status.pending',
  partial: 'status.partial',
  planned: 'status.planned',
  processing: 'status.processing',
  reconciliation_required: 'status.reconciliationRequired',
  policy: 'status.policy',
  policy_blocked: 'status.policyBlocked',
  present: 'status.present',
  preflight: 'status.preflight',
  preparing_context: 'status.preparingContext',
  queued: 'status.queued',
  ready: 'status.ready',
  received: 'status.received',
  reconciling: 'status.reconciling',
  recorded: 'status.recorded',
  registry: 'status.registry',
  required: 'status.required',
  reports: 'status.reports',
  retry_scheduled: 'status.retryScheduled',
  retrying: 'status.retrying',
  rework: 'status.rework',
  reply_retry_scheduled: 'status.replyRetryScheduled',
  replying: 'status.replying',
  safe: 'status.safe',
  scaned: 'status.scanned',
  scanned: 'status.scanned',
  scaned_but_redirect: 'status.confirming',
  sending: 'status.sending',
  spa: 'status.spa',
  static: 'status.static',
  started: 'status.started',
  starting: 'status.starting',
  stopped: 'status.stopped',
  stored: 'status.stored',
  thinking: 'status.thinking',
  loading: 'status.loading',
  running: 'status.running',
  seen: 'status.seen',
  server_error: 'status.serverError',
  stale: 'status.stale',
  streaming: 'status.streaming',
  submitted: 'status.submitted',
  superseded: 'status.superseded',
  suppressed_overlap: 'status.suppressedOverlap',
  unsupported: 'status.unsupported',
  usage: 'status.usage',
  warning: 'status.warning',
  warn: 'status.warning',
  unknown: 'status.unknown',
  valid: 'status.valid',
  verified: 'status.verified',
  waiting: 'status.waiting',
  waiting_approval: 'status.waitingApproval',
  awaiting_approval: 'status.waitingApproval',
  awaiting_producer: 'status.awaitingProducer',
  awaiting_resource: 'status.awaitingResource',
  waiting_for_scan: 'status.waitingForScan',
  withdrawn: 'status.withdrawn',
  expired: 'status.expired',
  low: 'status.low',
  medium: 'status.medium',
  materializing: 'status.materializing',
  reconnecting: 'status.reconnecting',
  terminal: 'status.terminal',
};

export function statusKeyFor(value: unknown): string {
  const normalized = String(value ?? 'unknown').trim().toLowerCase().replace(/[\s-]+/g, '_');
  return statusKeys[normalized] || 'status.unknownWithValue';
}

export function displayStatus(value: unknown): string {
  const raw = String(value ?? 'unknown');
  return t(statusKeyFor(raw), { value: raw });
}

export function displayBoolean(value: unknown): string {
  if (value === true || value === 'true' || value === 'yes') return t('boolean.yes');
  if (value === false || value === 'false' || value === 'no') return t('boolean.no');
  return t('status.unknownWithValue', { value: String(value ?? 'unknown') });
}
